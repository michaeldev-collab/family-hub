...
static uint8_t                s_bl_before_sleep = BL_DEFAULT_PERCENT;
...
static uint32_t s_wake_repair_until_ms = 0;  // re-assert BL after flaky wake

// CH32 IO-ext is picky on this bus: repeated-start reads often return the last
// written payload (e.g. PWM=204 appears as "mode=0xCC"), and single OUTPUT
// writes sometimes no-op. Use STOP between reg pointer and read, and verify.
...
static bool ioext_read1(uint8_t reg, uint8_t* out) {
  if (!out) return false;
  // Waveshare uses transmit+receive (repeated start). STOP-separated reads on
  // this CH32 often return the last written payload (PWM=204 appears as
  // "chip_out=0xCC"), which made wake look OK while DISP was still wrong.
  Wire.beginTransmission(IOEXT_ADDR);
  Wire.write(reg);
  if (Wire.endTransmission(false) != 0) return false;
  delayMicroseconds(50);
  if (Wire.requestFrom((int)IOEXT_ADDR, 1) != 1) return false;
  *out = Wire.read();
  return true;
}

static bool ioext_write2_verified(uint8_t reg, uint8_t val, int attempts = 5) {
  for (int i = 0; i < attempts; ++i) {
    if (!ioext_write2(reg, val)) {
      delay(2);
      continue;
    }
    if (reg == IOEXT_REG_PWM) {
      (void)ioext_write2(reg, val);
    ...
    delayMicroseconds(200);
    uint8_t got = 0;
    if (!ioext_read1(reg, &got)) {
      delay(2);
      continue;
    }
    // Reject obvious echo of a recent PWM duty (0xCC==204) when we wanted
    // a full OUTPUT mask like 0xFF/0xFB.
    if (reg == IOEXT_REG_OUTPUT && got == val) return true;
    if (reg == IOEXT_REG_OUTPUT && (got & (1u << IOEXT_IO_DISP)) == (val & (1u << IOEXT_IO_DISP)) &&
        (got & (1u << IOEXT_IO_LCD_VDD_EN)) == (val & (1u << IOEXT_IO_LCD_VDD_EN)) &&
        got != 0xCC && got != 0x00) {
      // DISP/VDD bits match even if other bits differ — good enough for BL.
      return true;
    }
    if (reg != IOEXT_REG_OUTPUT && got == val) return true;
    delay(2);
  }
  Serial.printf("[disp] IOEXT verify fail reg=0x%02X want=0x%02X\n", reg, val);
  return false;
}
...
static bool ioext_output(uint8_t pin, uint8_t value) {
  ...
  return ioext_write2_verified(IOEXT_REG_OUTPUT, s_ioext_shadow);
}

static bool ioext_init() {
  // Establish safe levels before switching the expander pins to outputs.
  s_ioext_ok = ioext_write2_verified(IOEXT_REG_OUTPUT, s_ioext_shadow) &&
               ioext_write2_verified(IOEXT_REG_MODE, 0xFF);
  return s_ioext_ok;
}

// Unstick shared GT911/IOEXT bus after a hung transaction (common after many
// sleep/wake cycles), then re-load expander MODE/OUTPUT.
static void ioext_bus_recover(const char* why) {
  Serial.printf("[disp] I2C recover (%s)\n", why ? why : "bus");
  Wire.end();
  delay(5);
  pinMode(BOARD_I2C_SCL, OUTPUT);
  pinMode(BOARD_I2C_SDA, INPUT_PULLUP);
  for (int i = 0; i < 9; ++i) {
    digitalWrite(BOARD_I2C_SCL, HIGH);
    delayMicroseconds(5);
    digitalWrite(BOARD_I2C_SCL, LOW);
    delayMicroseconds(5);
  }
  digitalWrite(BOARD_I2C_SCL, HIGH);
  delayMicroseconds(5);
  // STOP condition
  pinMode(BOARD_I2C_SDA, OUTPUT);
  digitalWrite(BOARD_I2C_SDA, LOW);
  delayMicroseconds(5);
  digitalWrite(BOARD_I2C_SCL, HIGH);
  delayMicroseconds(5);
  digitalWrite(BOARD_I2C_SDA, HIGH);
  delayMicroseconds(5);
...
static uint8_t backlight_pwm_raw(uint8_t percent) {
  uint8_t value = percent > 97 ? 97 : percent;
  if (value == 0) value = BL_DEFAULT_PERCENT;
  return (uint8_t)(value * 255U / 100U);
}

// Bring DISP (+ keep VDD) back with verify; recover the bus if needed.
static bool displayRestoreBacklight(uint8_t percent) {
  const uint8_t raw = backlight_pwm_raw(percent);
  for (int round = 0; round < 2; ++round) {
    (void)ioext_write2_verified(IOEXT_REG_MODE, 0xFF);
    for (int attempt = 0; attempt < 6; ++attempt) {
      // Duty first while gated off, then raise DISP, then re-poke duty.
      s_ioext_shadow |= (1u << IOEXT_IO_LCD_VDD_EN) | (1u << IOEXT_IO_LCD_RST);
      s_ioext_shadow &= ~(1u << IOEXT_IO_DISP);
      (void)ioext_write2_verified(IOEXT_REG_OUTPUT, s_ioext_shadow);
      (void)ioext_write2_verified(IOEXT_REG_PWM, raw);
      delay(5);
      s_ioext_shadow |= (1u << IOEXT_IO_DISP);
      if (!ioext_write2_verified(IOEXT_REG_OUTPUT, s_ioext_shadow)) {
        delay(10);
        continue;
      }
      delay(5);
      ...
      // Prefer INPUT sense for DISP pin — less confused with PWM echo.
      uint8_t in = 0, out = 0;
      const bool gotIn = ioext_read1(IOEXT_REG_INPUT, &in);
      const bool gotOut = ioext_read1(IOEXT_REG_OUTPUT, &out);
      const bool dispHigh = (gotIn && (in & (1u << IOEXT_IO_DISP))) ||
                            (gotOut && (out & (1u << IOEXT_IO_DISP)) && out != raw);
      if (dispHigh) return true;
      Serial.printf("[disp] BL restore retry r=%d a=%d out=%s0x%02X in=%s0x%02X "
                    "want_pwm=%u\n",
                    round, attempt, gotOut ? "" : "?", out, gotIn ? "" : "?", in,
                    raw);
      delay(15);
    }
    ioext_bus_recover("bl-restore");
  }
  return false;
}

// Backlight via CH32V003: EXIO_PWM duty + EXIO2 (DISP) enable.
// Waveshare IO_EXTENSION_Pwm_Output(Value): PWM_reg = Value*255/100 with
// Value clamped to 97 — their comment: clamping prevents the screen from
// "completely turning off" (AP3032: solid 100%/PWM=255 shuts the boost down).
// Official wavesahre_rgb_lcd_bl_off() only drives EXIO2 low.
void displaySetBacklight(uint8_t percent) {
  percent = constrain(percent, 0, 100);
  if (percent == 0) {
    // Official Waveshare bl_off is EXIO2 low. Do not park at PWM=255 here —
    // that AP3032 "boost off" duty made wake unreliable.
    ioext_output(IOEXT_IO_DISP, 0);
    return;
  }
  uint8_t value = percent;
  if (value > 97) value = 97;
  const uint8_t raw = (uint8_t)(value * 255U / 100U);
  if (!ioext_write2_verified(IOEXT_REG_PWM, raw)) {
    Serial.println("[disp] FAIL: backlight PWM write to CH32V003 @0x24 NACKed");
  }
  ioext_output(IOEXT_IO_DISP, s_backlight_ready ? 1 : 0);
}

static void displayLogIoext(const char* tag) {
  uint8_t out = 0, pwm = 0, mode = 0, in = 0;
  const bool gotOut = ioext_read1(IOEXT_REG_OUTPUT, &out);
  const bool gotPwm = ioext_read1(IOEXT_REG_PWM, &pwm);
  const bool gotMode = ioext_read1(IOEXT_REG_MODE, &mode);
  const bool gotIn = ioext_read1(IOEXT_REG_INPUT, &in);
  Serial.printf("[disp] %s shadow=0x%02X chip_out=%s0x%02X pwm=%s%u mode=%s0x%02X "
                "in=%s0x%02X DISP=%d VDD=%d\n",
                tag ? tag : "ioext",
                s_ioext_shadow,
                gotOut ? "" : "?", gotOut ? out : 0,
                gotPwm ? "" : "?", gotPwm ? pwm : 0,
                gotMode ? "" : "?", gotMode ? mode : 0,
                gotIn ? "" : "?", gotIn ? in : 0,
                (s_ioext_shadow >> IOEXT_IO_DISP) & 1,
                (s_ioext_shadow >> IOEXT_IO_LCD_VDD_EN) & 1);
}

// Sleep dark: official Waveshare bl_off = EXIO2 (DISP) low only.
// Do NOT cut LCD_VDD_EN — power-cycling the panel made wake fail ~1/5 times
// even with a reset pulse. Keep PWM parked at normal duty.
static void displayForceBacklightOff() {
  (void)ioext_write2_verified(IOEXT_REG_MODE, 0xFF);
  const uint8_t raw = backlight_pwm_raw(
      s_bl_before_sleep ? s_bl_before_sleep : BL_DEFAULT_PERCENT);
  for (int attempt = 0; attempt < 5; ++attempt) {
    (void)ioext_write2_verified(IOEXT_REG_PWM, raw);
    // Keep VDD + RST high; only gate the backlight enable.
    s_ioext_shadow |= (1u << IOEXT_IO_LCD_VDD_EN) | (1u << IOEXT_IO_LCD_RST);
    s_ioext_shadow &= ~(1u << IOEXT_IO_DISP);
    if (ioext_write2_verified(IOEXT_REG_OUTPUT, s_ioext_shadow)) break;
    delay(20);
  }
  displayLogIoext("sleep dark");
}

// Serial 'B': cycle kill combos so we can see which one actually darkens.
void displayBacklightProbe() {
  if (!s_ioext_ok) {
    Serial.println("[disp] BL probe: IOEXT not ready");
    return;
  }
  Serial.println("[disp] BL probe start — watch glass; report which step goes dark");
  const struct {
    const char* name;
    uint8_t pwm;
    uint8_t disp;
    ...
    {"1 DISP=0 only (Waveshare bl_off)", 204, 0, 1},
    {"2 PWM=0 DISP=1", 0, 1, 1},
    {"3 PWM=255 DISP=1 (AP3032 quirk)", 255, 1, 1},
    {"4 PWM=255 DISP=0", 255, 0, 1},
    {"5 PWM=0 DISP=0 VDD=0", 0, 0, 0},
    {"6 PWM=255 DISP=0 VDD=0 (sleep path)", 255, 0, 0},
    {"7 restore 80%", 204, 1, 1},
  };
  for (const auto& s : steps) {
    (void)ioext_write2_verified(IOEXT_REG_MODE, 0xFF);
    (void)ioext_write2_verified(IOEXT_REG_PWM, s.pwm);
    if (s.disp) s_ioext_shadow |= (1u << IOEXT_IO_DISP);
    else        s_ioext_shadow &= ~(1u << IOEXT_IO_DISP);
    if (s.vdd)  s_ioext_shadow |= (1u << IOEXT_IO_LCD_VDD_EN);
    else        s_ioext_shadow &= ~(1u << IOEXT_IO_LCD_VDD_EN);
    (void)ioext_write2_verified(IOEXT_REG_OUTPUT, s_ioext_shadow);
    Serial.printf("[disp] BL probe %s\n", s.name);
    displayLogIoext("probe");
    delay(3500);
  }
  Serial.println("[disp] BL probe done");
}
...
static void lvgl_touch(lv_indev_drv_t*, lv_indev_data_t* data) {
  ...
  } else if (frame == TouchFrameResult::NoFrame && s_touch_active &&
  ...
  data->state = s_touch_active ? LV_INDEV_STATE_PRESSED : LV_INDEV_STATE_RELEASED;
  data->point.x = last_x;
  data->point.y = last_y;
}
...
void displayTick() {
  ...
  displayLock();
  if (s_sleeping) {
    // Sample touch while asleep so double-tap / any-tap wake still works.
    static uint32_t last_pressed_frame_ms = 0;
    ...
    if (frame == TouchFrameResult::Pressed) {
      ...
      consecutive_errors = 0;
      s_touch_active = false;
    } else if (frame == TouchFrameResult::Error) {
      s_touch_active = false;
      if (++consecutive_errors >= 8) {
        Serial.println("[disp] sleep touch Error streak — reinit GT911");
        gt911_init();
        consecutive_errors = 0;
        last_pressed_frame_ms = 0;
        displayForceBacklightOff();
      }
    } else if (frame == TouchFrameResult::NoFrame && s_touch_active &&
               last_pressed_frame_ms != 0 &&
               millis() - last_pressed_frame_ms > 120) {
      s_touch_active = false;
    }
  } else {
...
bool displayTouchActive() { return s_touch_active; }
...
bool displayTouchReinit() {
  ...
  displayLock();
  const bool ok = gt911_init();
  s_touch_active = false;
  displayUnlock();
  Serial.printf("[disp] touch reinit %s\n", ok ? "ok" : "fail");
  return ok;
}

bool displayIsSleeping() { return s_sleeping; }

static void displayWaitFrame() {
  // Same path as LVGL flush: bounce/VSYNC ISR notifies this task when the
  // just-submitted frame has been scanned out. Bound so a missed IRQ cannot hang.
  if (s_lvgl_task_handle == xTaskGetCurrentTaskHandle()) {
...
static void displayPushBlackFullFrame(void* fb) {
  if (!s_panel || !fb) return;
  ulTaskNotifyValueClear(nullptr, ULONG_MAX);
  esp_err_t err = esp_lcd_panel_draw_bitmap(s_panel, 0, 0, PANEL_H_RES, PANEL_V_RES, fb);
  if (err != ESP_OK) {
    Serial.printf("[disp] sleep: draw_bitmap black failed err=0x%X\n", err);
    return;
  }
  displayWaitFrame();
}

// Black fill that survives PSRAM cache + bounce-buffer scanout. memset alone
// left white/UI edges because DMA still saw stale cache lines and/or BL went
// off mid-frame before black had been scanned.
static void displayBlankToBlack() {
  if (!s_panel) return;

  void* fb0 = nullptr;
  void* fb1 = nullptr;
  if (esp_lcd_rgb_panel_get_frame_buffer(s_panel, 2, &fb0, &fb1) != ESP_OK || !fb0) {
    Serial.println("[disp] sleep: failed to get framebuffers for black fill");
    return;
  }

  const size_t bytes = (size_t)PANEL_H_RES * PANEL_V_RES * sizeof(uint16_t);
  memset(fb0, 0, bytes);
  if (fb1) memset(fb1, 0, bytes);
  // Push CPU writes out of cache so RGB DMA / bounce copy sees real zeros.
  esp_err_t sync = esp_cache_msync(fb0, bytes, ESP_CACHE_MSYNC_FLAG_DIR_C2M);
  if (sync != ESP_OK) {
    Serial.printf("[disp] sleep: cache sync fb0 err=0x%X\n", sync);
  }
  if (fb1) {
    sync = esp_cache_msync(fb1, bytes, ESP_CACHE_MSYNC_FLAG_DIR_C2M);
    if (sync != ESP_OK) {
      Serial.printf("[disp] sleep: cache sync fb1 err=0x%X\n", sync);
    }
  }

  // Also paint via internal-SRAM strips (proven in FIRST_BOOT_DIAG) so any
  // bounce-buffer path gets fresh black tiles, not stale PSRAM edge lines.
  constexpr int kStrip = 10;
  static uint16_t strip[PANEL_H_RES * kStrip];
  memset(strip, 0, sizeof(strip));
  for (int y = 0; y < PANEL_V_RES; y += kStrip) {
    const int h = (y + kStrip <= PANEL_V_RES) ? kStrip : (PANEL_V_RES - y);
    esp_lcd_panel_draw_bitmap(s_panel, 0, y, PANEL_H_RES, y + h, strip);
  }
  delay(20);

  // Flip both framebuffers to black and wait for scan-out twice.
  displayPushBlackFullFrame(fb0);
  delay(20);
  if (fb1) {
    displayPushBlackFullFrame(fb1);
    delay(20);
  } else {
    displayPushBlackFullFrame(fb0);
    delay(20);
...
void displayEnterSleep() {
  if (!s_ready || s_sleeping) return;
  s_sleeping = true;
  s_wake_repair_until_ms = 0;
  s_bl_before_sleep = BL_DEFAULT_PERCENT;
  s_touch_active = false;
  ...
  delay(40);
  displayForceBacklightOff();
  Serial.println("[disp] sleep");
}

void displayWake() {
  if (!s_ready) return;
  // Always clear sleep + restore BL — even if firmware already thinks it's
  // awake (stuck-dark after a failed wake write).
  const bool was_sleeping = s_sleeping;
  s_sleeping = false;
  s_backlight_ready = true;
  s_wake_repair_until_ms = 0;

  const uint8_t pct = s_bl_before_sleep ? s_bl_before_sleep : BL_DEFAULT_PERCENT;
  if (!displayRestoreBacklight(pct)) {
    Serial.println("[disp] wake: BL restore failed after recover");
  }

  Serial.printf("[disp] wake%s\n", was_sleeping ? "" : " (unstick)");
  displayLogIoext("wake");
  displayForceFullRedraw();
}

void displayForceBacklightOn() {
  if (!s_ready) return;
  Serial.println("[disp] force backlight on");
  s_sleeping = false;
  s_backlight_ready = true;
  s_bl_before_sleep = BL_DEFAULT_PERCENT;
  s_wake_repair_until_ms = 0;
  if (!displayRestoreBacklight(BL_DEFAULT_PERCENT)) {
    Serial.println("[disp] force-on: BL restore failed after recover");
  }
  displayLogIoext("force-on");
}
...
void displayForceFullRedraw() {
  ...
  if (lv_layer_sys()) lv_obj_invalidate(lv_layer_sys());
  lv_refr_now(nullptr);
  displayUnlock();
}
...
void displayTick()                     {}
bool displayTouchActive()              { return false; }
bool displayTouchSample(uint16_t* x, uint16_t* y) {
  if (x) *x = 0;
  if (y) *y = 0;
  return false;
}
bool displayTouchReinit()              { return false; }
